  ...
  return (
    <div className="w-full h-96 overflow-y-scroll overflow-x-auto">
      <table className="w-full relative">
        {/* Table Heading */}
        <tr className="flex justify-between items-center flex-row text-xs  text-gray-600 border-y-[1px] sticky top-0 left-0 bg-white">
          <th className=" text-left font-normal  py-2 pl-6 cursor-pointer border-y-none flex absolute justify-center items-center ">
            <input
              type="checkbox"
              name=""
              id=""
              className="absolute right-8 z-1000"
              checked={isAllSelected}
              onChange={() => setIsAllSelected((prev) => !prev)}
            />
          </th>
          <th className="w-full  text-left font-normal hover:bg-gray-200 py-2 pl-2 cursor-pointer">Name</th>
